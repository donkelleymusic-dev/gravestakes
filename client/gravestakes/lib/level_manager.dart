...
class LevelManager {
  ...
  static LevelConfig getConfigForLevel(int level) {
    if (level <= 2) {
      ...
      // 3 bots, slightly faster.
      return LevelConfig(botCount: 3, wanderSpeed: 70.0, huntSpeed: 115.0);
    } 
    else if (level <= 10) {
      // Levels 6-10: Standard Difficulty
      // 4 bots, normal speeds.
      return LevelConfig(botCount: 4, wanderSpeed: 80.0, huntSpeed: 130.0);
    } 
    else if (level <= 20) {
      // Levels 11-20: Hard Mode
      // 5 bots, hunting speed is now significantly threatening.
      return LevelConfig(botCount: 5, wanderSpeed: 90.0, huntSpeed: 145.0);
    } 
    else {
